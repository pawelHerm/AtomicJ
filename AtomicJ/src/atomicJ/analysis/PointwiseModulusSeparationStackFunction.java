...
public class PointwiseModulusSeparationStackFunction implements ProcessedStackingPackFunction
{
    private final double depth;
    private final double sampleHeighestPoint;

    public PointwiseModulusSeparationStackFunction(double depth, double sampleHeighestPoint)
    {
        this.depth = depth;
    ...
    public double evaluate(ProcessedSpectroscopyPack pack) 
    {
        ...
        return pointwiseModulus[index][1];
    }
    ...
    public static class PointwiseModulusSeparationStackFunctionFactory implements StackingPackFunctionFactory<PointwiseModulusSeparationStackFunction>
    {
        private static final String stackType = Datasets.POINTWISE_MODULUS_STACK;

        private final double sampleHeighestPoint;

        public PointwiseModulusSeparationStackFunctionFactory(double sampleHeighestPoint)
        {
            this.sampleHeighestPoint = sampleHeighestPoint;
        }
        @Override
        public PointwiseModulusSeparationStackFunction getFunction(double level) 
        {
            return new PointwiseModulusSeparationStackFunction(level, sampleHeighestPoint);
        }
