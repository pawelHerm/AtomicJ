...
public class NIDImageReader extends AbstractSourceReader<ImageSource>
{  
    private static final String HEADER_END = "#!";

    private static final String[] ACCEPTED_EXTENSIONS = new String[] {"nid"};
    private static final String DESCRIPTION = "Nanosurf image (.nid) (v2)";
    ...
    {           
        ...
        {                   
            ...
            int elementCount = dataset.getImageElementCount();
            SourceReadingState state  = elementCount > 2  ? new SourceReadingStateMonitored(elementCount, SourceReadingStateMonitored.IAMGE_PROBLEM) : new SourceReadingStateMute(elementCount);   

            try{
                List<ImageSource> readInSources = dataset.readInImageDataAndSkipOthers(channel, f, readingDirectives, state);
                imageSources.addAll(readInSources);
            }
            catch (Exception e) 
            {
                state.setOutOfJob();
                throw new UserCommunicableException("Error has occured while reading a NID image", e);
            }  
        } 
        catch (Exception e) 
        {
