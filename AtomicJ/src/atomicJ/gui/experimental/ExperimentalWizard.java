...
public class ExperimentalWizard extends JDialog implements PropertyChangeListener, MessageDisplayer
{
    ...
    private final ExperimentalWizardModel wizardModel;

    public ExperimentalWizard(String title, PreviewDestination parent, ROIWizardReceiver roiImageReceiver, ExperimentalWizardModel model)
    {
        super(parent.getPublicationSite(), title, ModalityType.MODELESS);
        ...
        addWindowListener(new WindowAdapter() 
        {
            ...
            {
                ExperimentalWizard.this.cancel();
            }
        });
        ...
        pack();
        setLocationRelativeTo(parent.getPublicationSite());
    }

    public boolean showDialog(File defaultSourceLocation)
    {
        reset();
        setVisible(true);
        wizardModel.setDefaultSourceLocation(defaultSourceLocation);
        return wizardModel.isApproved();
    }

    public void setROIImageReceiver(PreviewDestination previewDestination, ROIWizardReceiver roiImageReceiver)
    {
        wizardModel.setROIReceiver(previewDestination, roiImageReceiver);
    }
    ...
    public void propertyChange(PropertyChangeEvent evt) 
    {
        ...
        else if(WizardModelProperties.BACK_COMMAND.equals(name))
        {
            String backCommand = evt.getNewValue().toString();
            backAction.putValue(Action.NAME, backCommand);
            buttonBack.setAction(backAction);
        }
        else if(WizardModelProperties.NEXT_COMMAND.equals(name))
        {
            String nextCommand = evt.getNewValue().toString();

            nextAction.putValue(Action.NAME, nextCommand);
            buttonNext.setAction(nextAction);
        }
    }
