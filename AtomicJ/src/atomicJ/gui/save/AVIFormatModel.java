...
import static atomicJ.gui.save.SaveModelProperties.AVI_COMPRESSION;
...
public class AVIFormatModel extends BasicFormatModel 
{
    ...
    private AVICompression compression = AVICompression.NO_COMPRESSION;

    public AVISaverNIO getChartSaver(Component parent)
    {
        ...
        AVISaverNIO saver = new AVISaverNIO(chartInitialArea, width, height, firstFrame, lastFrame, frameRate, backwards, saveDataArea, compression, 100*(int)Math.rint(quality.getQuality()));
    ...
    public void setCompression(AVICompression compressionNew) 
    {
        AVICompression compressionOld = compression;
        this.compression = compressionNew;

        firePropertyChange(AVI_COMPRESSION, compressionOld, compressionNew);
    }
}